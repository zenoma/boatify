defmodule ClientboaterTest do
  ...
  test "full_client_functions", _value do
    ...
    assert Kernel.match?( info_trip,["BoaterTest","ModeloTest", "FechaTest", "RutaTest", "Hora","4","4","Open" ])

    Clientboater.enviar_boater(3, id)
    Process.sleep 500
    
    [idc|info_tripc] = "./Trip.csv"
    |> Path.expand()
    |> File.stream!()
    |> CSV.decode()
    |> Enum.map(fn x -> elem(x, 1) end)
    ...
    assert Kernel.match?( info_trip,["BoaterTest","ModeloTest", "FechaTest", "RutaTest", "Hora","4","4","Canceled" ])
